...
- **Raw evidence vs derived views.** Raw evidence comes in two kinds, and the difference matters for what history can be recovered:
  ...
  - **Mutable source records** are updated in place as state advances: `human_actions` (`resolved_at`, status), `queue_entries` (`state`, and `wait_reason`/`wait_reason_at`, which are overwritten on each change and cleared on admit and on operator dequeue, but retained as the latest value by terminal-issue housekeeping removal), and `worker_sessions` (`heartbeat_at`, `completed_at`, process columns). They hold only their *latest* state, so a value read from them is a snapshot: history that was overwritten is not recoverable from them, and a boundary taken from one is a proxy unless the [source matrix](#6-source-matrix) says otherwise.
...
| `queue_wait` end | `queue.admitted` / `queue.removed` | `queue_entries.state` flips, but **no terminal timestamp** is stored. `wait_reason`/`wait_reason_at` are cleared on admit (`markQueueEntryAdmitted`) and on operator dequeue (`dequeueIssue`); terminal-issue housekeeping (`markQueueEntryRemoved`) only sets `state = 'removed'` and leaves the latest reason in place. Either way the removal path is not distinguishable from admission by timestamp, and only the latest reason survives | **Needs new event/schema** — `unavailable` for history |
