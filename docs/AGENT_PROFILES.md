...
- **Logs.** The session log (`…-developer-<ts>.ndjson`) holds Claude/Cursor-shaped events derived from Muse's stream, so the Issue Detail strip, usage-cap and auth classifiers work unchanged. Muse's untouched stdout is archived beside it as `….muse-raw.jsonl`.
