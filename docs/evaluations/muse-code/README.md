...
| muse-06 (4 commands) | held-out non-zero (clean tree: shared build exits 2; prebuilt dist: 6 fail); typecheck non-zero with held-out files in place; forwarding check exit 1 (0/1 pass); web form check exit 1 (1/4 pass) | held-out pass 33; typecheck exit 0; forwarding check pass 1; web form check pass 4 |
...
A quantity the vendor has no tier for is `0` by the mapping; a quantity that has a tier and is not reported is `null`. For Muse Code the adapter does not exist, so the operator records the exact raw field paths (`candidate.tokenMapping.rawFields.input/cache_read/cache_write/output`; `cache_write` may be `"none"`), whether input includes cache (`inputIncludesCached`) and the attestation `mappingRecordedInRunPlan: true` before run 1, and `--ready` fails without all of them; if the log or Meta's documentation does not establish it, nothing is guessed and the candidate's cost is `null`.
...
git diff --check
node scripts/validate-muse-code-manifest.mjs --ready   # pre-run gate: fails until the operator confirms the candidate model and records its token mapping (attestation flag, cached-input semantics, exact raw field paths)
```
