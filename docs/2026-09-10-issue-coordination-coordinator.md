...
**Amendments (found running this task; the plan's own Step 3 code, as flagged by its "Note for the implementer," did not match the real adapter/schema):**

1. **Real bug — the plan's `evidence` payload fails `tsc --noEmit` under this repo's `strict: true`.** `proposePlaybookPatch`'s third argument is typed `ReflectProposal & { playbook_id: string }` (`agent-deck.ts`), and `ReflectProposal`'s `evidence` field is `PatchEvidenceContent.optional()` where `PatchEvidenceContent` (`packages/shared/src/playbook-reflect.ts`) requires `failure_summary: z.string().min(1)` and `user_feedback_excerpt: z.string().min(1)` — both **non-optional strings**, not `string | undefined`. The plan's code passed `evidence: { failure_summary: undefined, user_feedback_excerpt: undefined }`, which does not type-check. Verified empirically: swapping the plan's exact code back in and running `npx tsc --noEmit -p packages/server/tsconfig.json` reproduces `TS2322: Type 'undefined' is not assignable to type 'string'` at both fields (exit code 2). Fixed by dropping the `evidence` field entirely — it's optional at the top level, and this trigger has no real failure-summary/feedback-excerpt text to report (those only exist for the legacy `runReflect`'s `"retry"` trigger path), so omitting it is the correct match for the real schema, not a workaround.

2. **Real bug — `ops: []` violates `ReflectProposalSchema`'s `ops: z.array(ReflectPatchOpSchema).min(1)`.** This doesn't surface as a TypeScript error (arrays have no compile-time minimum length), so it's a silent runtime/domain gap: the "Note for the implementer" was correct to flag it. Since this trigger deliberately does not spawn a Claude session to derive structured deltas the way `runReflect` does (an explicitly acknowledged simplification, per the plan's own Self-Review Notes), there is no agent-generated `ops` array to send. Fixed by sending exactly one `add_item` op recording the completion rationale (`ops: [{ op: "add_item", section: "Notes", text: rationale }]`) — satisfies the schema's `min(1)` without fabricating a structured playbook edit the trigger has no basis for proposing. This keeps the acknowledged "simplified variant, not full parity" framing from the plan intact: it now produces a schema-valid but intentionally minimal patch proposal, rather than one that would be rejected outright by the real API.

3. **`checkAgentDeckHealth`, `fetchPlaybook` signatures matched the plan as written.** `checkAgentDeckHealth(): Promise<boolean>` and `fetchPlaybook(playbookId: string): Promise<{ id: string; title: string; body: string }>` (`agent-deck.ts`) are called exactly as the plan's Step 3 code calls them — no changes needed there. `proposePlaybookPatch(deckId: string, runId: string, proposal: ReflectProposal & { playbook_id: string })`'s first two positional arguments (`developerAgentDeckId`, `issueId`) also matched the plan and `reflect.ts`'s existing call pattern (`proposePlaybookPatch(run.deckId, run.id, {...})`) — only the third argument's payload shape needed correction, per points 1 and 2 above.

Confirmed the fix compiles clean (`npx tsc --noEmit -p packages/server/tsconfig.json` exits 0) and the single "skipped" test still passes — that test never reaches the `proposePlaybookPatch` call (`developerAgentDeckId`/`developerAgentPlaybookId` are both `null`), so neither the original bug nor the fix was ever visible to it; the type error was only caught by `tsc`, not by `npx tsx --test`, which does not type-check.
