...
**Amendment (found running this task; a schema bug inherited from Plan 1, not Task 1's own code, but one that blocks Task 1's own test as written):**

1. **Real bug — `artifacts.run_id` was `NOT NULL`, but issue-linked artifacts have no run.** Step 1's own test inserts an artifact with `run_id = NULL` (correct: an issue-linked artifact shouldn't require a legacy `run`), but `schema.sql` still declared `run_id TEXT NOT NULL REFERENCES runs(id)`. Reproduced empirically before writing the implementation (a standalone insert against the real schema threw `NOT NULL constraint failed: artifacts.run_id`), then confirmed again at Step 4 after `artifacts-for-issue.ts` existed: 6/7 tests passed, with exactly the "lists artifacts for an issue" test failing with `SQLITE_CONSTRAINT_NOTNULL`. Plan 1's migration (`packages/server/src/db/index.ts`) had already added the nullable `issue_id`/`worker_session_id` columns to `artifacts` with a comment claiming "`run_id` stays NOT NULL for legacy rows" — true for legacy rows, but it never accounted for new issue-linked rows, which have no run to reference and can't satisfy that constraint. This isn't scoped to Task 1's file list (`artifacts-for-issue.ts`, `usage-events.ts`, `intent-forecast.ts`), but the plan's own Task 1 test cannot pass against the real schema without it, so it was fixed as part of this task rather than deferred. Fixed with the smallest schema change that preserves both write paths: (a) `schema.sql`'s `artifacts` table now declares `run_id TEXT REFERENCES runs(id)` (nullable) for fresh databases; (b) `db/index.ts`'s `migrate()` gained a new block, placed right after the existing `issue_id`/`worker_session_id` additive-column block, that checks `PRAGMA table_info(artifacts)` for `run_id`'s `notnull` flag and — only if still `1` (i.e., an existing database created before this fix) — rebuilds the table (`CREATE TABLE artifacts_new` without the `NOT NULL`, copy, drop, rename, recreate `idx_artifacts_run`) since SQLite has no `ALTER COLUMN DROP NOT NULL`. Also corrected the now-inaccurate "`run_id` stays NOT NULL for legacy rows" comment on the neighboring `issue_id`/`worker_session_id` block, since that's no longer true. Verified the migration path (not exercised by any existing test, since test databases are always freshly created from the now-fixed `schema.sql`) by manually building a pre-fix-equivalent database (temporarily reverting just the `schema.sql` line, running `migrate()` to create it, inserting a legacy run-linked artifact row, then restoring the fixed `schema.sql` and calling `migrate()` again): the rebuild ran, the legacy row's data was preserved byte-for-byte, `run_id`'s `notnull` flag flipped to `0`, a second `migrate()` call was a clean no-op, and a subsequent `NULL`-`run_id` insert succeeded. `packages/server/src/db/schema.test.ts` and `packages/server/src/db/artifacts-issue-columns.test.ts` (both pre-existing, neither in Task 1's file list) still pass unchanged, and the full server suite is 165/165.
...
**Amendment (found running this task; a bug in this task's own Step 3 prose):**

1. **Real bug — the plan's `try { resolveHumanAction(...) } catch { … }` pattern cannot detect a double-resolve, and unconditionally re-running the side effects on a second resolve crashes.** `resolveHumanAction` (Task 4's own "Consumes" list, from Plan 1/3's `repository/human-actions.ts`) only throws when the action id doesn't exist at all — its `UPDATE ... WHERE id = ? AND status = 'open'` is a silent no-op when the action is already resolved, and the subsequent `SELECT` still finds the row (with `status = 'resolved'` from the *first* call) and returns it without throwing. So the plan's Step 3 code, which treats "the catch fired" as its only idempotency signal, would fall through to the full side-effect block on a second resolve of the same action. Reproduced empirically before fixing: Step 1's own third test ("resolving an already-resolved action returns its resolved state instead of erroring") transitions the issue to `done` on the first resolve; the plan's code then calls `transitionIssue(issue.id, "done", ...)` again on the second resolve, and `ISSUE_STATUS_TRANSITIONS.done` (Plan 1, `packages/shared/src/issues.ts`) is `[]` — a terminal state with no self-transition — so `canTransitionIssue("done", "done")` returns `false` and `transitionIssue` throws `Invalid transition: done → done`, which propagates as an unhandled exception and would 500 the route instead of returning the expected 200. Verified the state-transition table directly (`canTransitionIssue("done","done")` → `false`) before writing the fix. Fixed by checking `listOpenHumanActions().some((a) => a.id === id)` *before* calling `resolveHumanAction`, and skipping the entire side-effect block (transitionIssue, workflow completion, round increment, reflect trigger) when the action was not open at call time — returning the already-resolved action as-is instead, which is both correct (no double round-increment or double reflect-trigger either) and matches the spec's "idempotently returns" framing used elsewhere in this plan. The `try/catch` around `resolveHumanAction` itself is kept, but now only for the genuine 404 case (unknown action id). All 4 of Task 4's tests, including the double-resolve one, pass with this fix; the full server suite is 182/182.
