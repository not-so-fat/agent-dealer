...
- `POST /api/issues` — create/import (manual or Linear). Guarded on *live* `(source, external_id)` only (NOT-141): a match that is still startable is re-enqueued and answers `created: false` with the queue outcome it actually caused (`enqueued`, or `already_queued` when it was waiting already), any other live match answers 409 with `existingIssueId`, and a ticket whose only issues are terminal gets a new row (`priorPasses > 0`) so a second pass is possible
