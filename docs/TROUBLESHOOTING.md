...
fails loudly. What decides the split is *where* the error came from, not how it reads: only
a call that never got an answer can be unavailable. That matters because a deck that is up
can report its own proxied service as down (NOT-101) with the literal text `fetch failed` —
classifying on prose would have waited forever on a deck error. The deferral reuses the
NOT-111 usage-cap path (`deferWorkItem` + `revertAttemptCount`), so the claim-time
`attempt_count` bump is undone too.
...
| `DECK_OUTAGE_DEFERRAL_CEILING_MS` | `86400000` | How long one item may wait before the outage stops being "a restart" and escalates to a human. |
