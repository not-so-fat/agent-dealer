...
test("lists artifacts for an issue newest first, respects limit", () => {
  const issue = createIssue({ title: "T", repo: "acme/app", baseBranch: "main", developerAgentId: BUILTIN_AGENT_CLAUDE_ID, reviewerAgentId: BUILTIN_AGENT_CURSOR_ID, maxReviewRounds: 3, maxInfraAttempts: 3, source: "manual" });
  const db = getDb();
  const now = Date.now();
  for (let i = 0; i < 3; i++) {
    db.prepare(
...
test("listArtifactsForIssueByKind finds a matching row regardless of how many newer rows of other kinds exist", () => {
  const issue = createIssue({ title: "K", repo: "acme/app", baseBranch: "main", developerAgentId: BUILTIN_AGENT_CLAUDE_ID, reviewerAgentId: BUILTIN_AGENT_CURSOR_ID, maxReviewRounds: 3, maxInfraAttempts: 3, source: "manual" });
  const db = getDb();
  const now = Date.now();
  db.prepare(
...
test("listUsageEventsForIssue returns recorded events", async () => {
  ...
  const issue = createIssue({ title: "U", repo: "acme/app", baseBranch: "main", developerAgentId: BUILTIN_AGENT_CLAUDE_ID, reviewerAgentId: BUILTIN_AGENT_CURSOR_ID, maxReviewRounds: 3, maxInfraAttempts: 3, source: "manual" });
