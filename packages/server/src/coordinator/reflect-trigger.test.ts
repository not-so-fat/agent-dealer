...
test("skips when the developer profile has no deck configured", async () => {
  const issue = seedIssue(BUILTIN_AGENT_CLAUDE_ID); // built-in agent: no deck/playbooks
...
test("posts one patch per playbook, using the implementation conclusion and review history as rationale", async () => {
  const dev = createAgent({ name: `dev-${Math.random()}`, runtime: "claude_code", workspaceRoot: "/repo", deckId: "11111111-1111-4111-a111-111111111111", playbookIds: ["pb-1", "pb-2"] });
  const issue = seedIssue(dev.id);
...
test("a failed patch proposal for one playbook is recorded, not thrown, and does not block the others", async () => {
  const dev = createAgent({ name: `dev-${Math.random()}`, runtime: "claude_code", workspaceRoot: "/repo", deckId: "11111111-1111-4111-a111-111111111111", playbookIds: ["pb-ok", "pb-fail"] });
  const issue = seedIssue(dev.id);
  const deps = {
    checkHealth: async () => true,
