...
- **Configurable Linear label-to-repository mappings (NOT-260)** — repository resolution for a Linear ticket now also checks a persisted, editable label-to-repository mapping (`GET`/`PUT /api/settings/repository-mappings`, inline settings editor, shared picker) before falling back to the existing `repo:` label convention; a matched mapping overrides a `repo:` label when both are present, and the old `repo:` convention still applies unchanged when no configured mapping matches the ticket's labels.
